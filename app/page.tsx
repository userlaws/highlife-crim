...
import { fetchScores, formatTime, titleToGame, type Score } from '@/lib/leaderboard';
import { gameCatalog } from '@/lib/games';

type Game = 'DES Code Match' | 'Vault Drill' | 'Data Crack' | 'VOLTlab';


const games: { title: Game; description: string; time: string; type: string }[] = [
  { title: 'DES Code Match', description: 'Match the sequence before the timer expires.', time: '15s', type: 'code' },
      ...
      <main className="content" id="top">
        ...
        <section className="site-section" id="leaderboard">
          <div className="section-header leaderboard-title">
            <div><h2>Leaderboard</h2><p>Best times from the community.</p></div>
          ...
          <div className="score-table">
            <Table className="leaderboard-table">
              <TableHeader><TableRow><TableHead>#</TableHead><TableHead>Player</TableHead><TableHead>Minigame</TableHead><TableHead>Best Time</TableHead></TableRow></TableHeader>
              <TableBody>
                {leaders.map((leader) => (
                  <TableRow key={`${leader.rank}-${leader.player}`}>
                    <TableCell><strong className={`rank rank-${leader.rank}`}>{leader.rank}</strong></TableCell>
                    <TableCell>{leader.player}</TableCell><TableCell>{gameCatalog[leader.game].title}</TableCell><TableCell><time>{formatTime(leader.ms)}</time></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <div className="practice-grid">
            {games.map((game) => (
              <Card className="practice-card" key={game.title}>
                ...
                <div className="game-copy"><h3>{game.title}</h3><p>{game.description}</p><small>Timer preset: <strong>{game.time}</strong></small></div>
